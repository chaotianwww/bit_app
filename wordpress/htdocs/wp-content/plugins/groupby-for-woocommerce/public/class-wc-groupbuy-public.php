        ...
        if (method_exists( $product, 'get_type') && $product->get_type() == 'groupbuy' ) {
            ...
            if ( $max_deals_per_user == false ) {
             ...
             if ( !is_user_logged_in() ) {
                ...
                wc_add_notice(sprintf(__('Sorry, you must be logged in to participate in Group Buy deal. <a href="%s" class="button">Login &rarr;</a>', 'wc_groupbuy'), get_permalink(woocommerce_get_page_id('myaccount'))), 'error');
                return false;
            }
        ...
        foreach ( wc()->cart->get_cart() as $cart_item_key => $values ) {
            ...
            if (method_exists( $product, 'get_type') && $product->get_type() == 'groupbuy'){
                ...
                if (!is_user_logged_in()) {
                    ...
                    wc_add_notice(sprintf(__('Sorry, you must be logged in to participate in Group Buy deals. <a href="%s" class="button">Login &rarr;</a>', 'wc_groupbuy'), get_permalink(woocommerce_get_page_id('myaccount'))), 'error');
