...
class Gens_RAF_Public {
    ...
    public function account_page_show_order_link($order_id,$items) {

        $referral_id = $this->get_referral_id( get_current_user_id() );
        $refLink = esc_url(add_query_arg( 'raf', $referral_id, get_home_url().'/product/testtestest?order_id='.$order_id."&user_id=".get_current_user_id().'&items='.$items ));
        ?>
