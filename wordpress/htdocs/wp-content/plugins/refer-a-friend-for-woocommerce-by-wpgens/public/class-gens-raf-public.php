...
class Gens_RAF_Public {
    ...
    public function account_page_show_order_link($orders) {
        ...
        $order = [];
        print_r($orders);
        foreach($orders as $tmp_item_id => $val){
            $order =  $val->get_product();
        }
        $referral_id = $this->get_referral_id( get_current_user_id() );
        $refLink = esc_url(add_query_arg( 'raf', $referral_id, get_home_url().'/product/'.$order->get_name().'?item_id='.$order->get_product_id().'&order_id='.$order->get_order_id()."&user_id=".get_current_user_id() ));
        ?>
